...
You can also see that the output is set to `vitae::awesomecv`, which indicates that this CV uses the [Awesome CV](https://github.com/posquit0/Awesome-CV) template. Some of the templates allow for the choosing of a theme.  For example, the `moderncv` template allows for the selection of one of five themes:  "casual", "classic", "oldstyle", "banking" or "fancy". An author selecting a theme, other than the default, might modify the YAML header to:

```{r Output-theme, eval=F}
#choose theme
output:
  vitae::moderncv:
    banking
