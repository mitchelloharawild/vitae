...
For example, if your document contained this YAML header:
```
...
The moderncv template `\cvitem` command seems generate an appropriate layout for brief entries. It expects inputs in this format:
...
So in our pandoc template's header, we need to define a macro for `\briefsection` and `\briefitem` that produces an appropriately structured `\cvitem`. The `\briefitem` provides 3 inputs, and `\cventry` expects 2, so we will need to combine two of the `\briefitem` inputs. A suitable LaTeX macro could be defined as:
...
For detailed CV entires, the moderncv `\cventry` command is reasonable. It expects inputs in this format:
...
usethis::use_rmarkdown_template(
  ...
  template_dir = "moderncv",
  template_description = "The ModernCV template (https://github.com/xdanaux/moderncv) for vitae",
  template_create_dir = TRUE)
```

Then by navigating to the package's `inst/rmarkdown/templates/moderncv` folder, you can add your Pandoc template to the `resources` folder, and your `.cls` and `.sty` files to the `skeleton` folder.

Once that is done, we can create a new `rmarkdown` output format that uses your template. These are added to the "R/formats.R" file, and will usually follow the same structure as other templates. The template argument to `cv_document` is a link your Pandoc template in the package (accessed using `system.file`), and it is recommended that the supporting `.cls` and `.sty` files are copied using `copy_supporting_files`.
